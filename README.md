...
The native YOLOv11n-seg uses 1024-dimensional channels in the deep backbone and neck. _Max Channels_ below denotes the largest base channel width specified in the backbone (the P5/SPPF stage), before the Ultralytics `scales` width multiplier is applied. The proposed setting (**768**) gives the optimal accuracy–efficiency trade-off, reducing FLOPs by 25.77% (9.7 → 7.2 GFLOPs) at only a 0.9 percentage-point mAP cost. The 640 (over-pruned) and 1024 (unpruned) variants are released for comparison.
